...
First, Table \ref{tbl:var-definitions} below briefly recapitulates the variables over which a causal inference problem-instance is defined and their associated definitions. Full explanations for these variables are presented in the theory chapter above.\par
...
A Monte Carlo evaluation design is specified by the selection of four functions which produce and relate these variables:\par


\vspace{\baselineskip}
\begin{itemize}
	\item  \(  \rho :  \left(  \right)   \rightarrow R^{ \left( n, d \right) } \) \  - the \textbf{covariate population sampler} which represents the joint distribution over the  \( d \)  covariates and produces samples of the covariate matrix  \( X. \) Strictly speaking,  \( X \) contains sampled covariate observations for  \( n \) individuals such that a ‘sampled’  \( X \) is constructed by repeatedly sampling the underlying joint distribution over the covariates.\par

	\item  \(  \Omega :~R^{ \left( n, d \right) }~ \rightarrow R^{n} \) \  - the \textbf{treatment assignment sampler} which assigns a treatment status  \( T \) to the individuals in  \( X \) .\par

	\item  \(  \Phi :~R^{ \left( n, d \right) }~ \rightarrow R^{n} \) \  - the \textbf{outcome} \textbf{sampler} which assigns an outcome under control ( \( Y_{0} \) ) to the individuals in  \( X \) .\par

	\item  \(  \tau:~R^{ \left( n, d \right) }~ \rightarrow R^{n} \) \  - the \textbf{treatment effect} \textbf{sampler} which assigns a treatment effect ( \(  \tau \) ) to the individuals in  \( X \) .
\end{itemize}\par


\vspace{\baselineskip}
The full DGP is then given by the following procedure defined over the variables and functions above:\par


\vspace{\baselineskip}
 \( X \leftarrow  \rho  \left(  \right)  \) \par

 \( T \leftarrow  \Omega  \left( X \right)  \) \par

 \( Y_{0} \leftarrow  \Phi  \left( X \right)  \) \par

 \( TE \leftarrow  \tau \left( X \right)  \) \par

 \( Y_{1}~ \leftarrow Y_{0}+ TE \) \par

 \( Y = T \times Y_{1}+  \left( 1-T \right)  \times Y_{0} \) \par
